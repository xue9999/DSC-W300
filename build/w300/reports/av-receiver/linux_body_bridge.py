"""Decode three hash-pinned G3 routines as data; never execute firmware code."""
...
SELECTED = {'libsencore.so': ['_ZN22AVCommunicationCommand9DoExecuteEP12SenserPacket', '_ZN10MemoryDataC1EPFiPhjEPvjPFvS3_E', '_Z9SerializePFiPhjEPvj']}
...
for item in inventory:
    ...
    dynrel=named['.rel.dyn']
    dynsyms=sections[dynrel[6]]
    dynstr=sections[dynsyms[6]]
    dynstr=data[dynstr[4]:dynstr[4]+dynstr[5]]
    selected_slots={0x1b53c:'header_receive',0x1b590:'received_count',0x1b5e8:'body_receive',0x1b5dc:'response_send'}
    bridge={}
    for at in range(dynrel[4],dynrel[4]+dynrel[5],dynrel[9]):
        slot,info=struct.unpack_from('<II',data,at)
        if slot in selected_slots:
            assert info & 255 == 21
            name_at,value,size=struct.unpack_from('<III',data,dynsyms[4]+(info>>8)*dynsyms[9])
            bridge[selected_slots[slot]]={'got_slot':hex(slot),'symbol':dynstr[name_at:].split(b'\0',1)[0].decode(),'value':hex(value)}
    assert len(bridge)==4
    (HERE/'linux-body-bridge-evidence.json').write_text(json.dumps(bridge,indent=2)+'\n',encoding='utf-8')
    ...
    for requested in SELECTED[name]:
        if requested == 'local-category-helper':
            ...
            symbol = dict(symbol)
            # Confirmed final returns precede literal pools included in ELF symbol sizes.
            code_ends = {'_ZN22AVCommunicationCommand9DoExecuteEP12SenserPacket':0xf1c8,
                         '_ZN10MemoryDataC1EPFiPhjEPvjPFvS3_E':0x97fc,
                         '_Z9SerializePFiPhjEPvj':0x67f4}
            symbol['size'] = code_ends[requested] - int(symbol['value'],16)
...
destination = HERE / 'linux-body-bridge.asm.txt'
