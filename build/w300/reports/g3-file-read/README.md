...
Reading file contents does not establish that the session leaves device state unchanged: file access timestamps, diagnostic logging and service entry/exit may change state. Do not use the general PMCA shell or its backend initialization as if they were an isolated file getter.
...
After the preceding W300 session qualification, the first candidate for a later probe is a bounded command-2 read of the one reviewed regular-file path. If the result shows that the path is missing or inaccessible, stop pursuing that path. Use the recorded responses to distinguish a path problem from a transport failure, then consult W300 boot/mount or package evidence for another regular-file candidate. Continue external package/trace acquisition if no such evidence is available. Broad path guessing, device-file reads, terminal activation and memory probes require different evidence and are outside this candidate. If a regular-file transfer qualifies, acquire actual W300 libraries and script containers, then validate their complete structure and repeat-read equality before analyzing them.

The immediate offline work is to trace G3 device-side segment/padding behavior, define receiver checks for short, empty and oversized responses, and document the evidence needed for W300 entry and exit. The [execution plan](../../../../docs/w300/EXECUTION_PLAN.md) coordinates that preparation with package and trace acquisition; lack of a connected camera limits the live trial, not these preparation tasks.
