...
static int usb_event_ioctl(struct inode*, struct file*, unsigned in, unsigned long);
...
#if !(defined(CONFIG_ARCH_CXD4105BASED) && defined(CONFIG_USB_EVENT))
static
#endif
                   ...
                   int *eof, void *data)
{
    ...
    len += sprintf(buf+len, "count                : %ld\n", g_count);
    len += sprintf(buf+len, "err_count            : %ld\n\n", g_err_count);
    
    len += sprintf(buf+len, "queue.buf_size       : %d\n", g_obj.queue.buf_size);
    len += sprintf(buf+len, "queue.data_size      : %d\n", g_obj.queue.data_size);
    len += sprintf(buf+len, "queue.max_data_size  : %d\n\n", g_obj.queue.max_data_size);
    
    len += sprintf(buf+len, "hdl_list.num_element : %d\n", g_obj.hdl_list.num_element);
    len += sprintf(buf+len, "hdl_list.num_data    : %d\n", g_obj.hdl_list.num_data);
    for(i = 0; i < g_obj.hdl_list.num_element; i++){
        if(g_obj.hdl_list.hdls[i]){
            len += sprintf(buf+len, " handle[%2d]:%08d\n", i, (int)g_obj.hdl_list.hdls[i]);
        }
        if(len > (count - 80)){
            len += sprintf(buf+len, "[more]\n");
        }
    }
...
static struct sonymisc_device usb_event_device = 
{
    .class  = DEV_CLASS_CHR,
    .minor  = USBEVENT_MINOR_NUMBER,
    .name   = USBEVENT_NAME,
    .fops   = &usb_event_fops,
    .gd     = NULL,
};
...
int __init usb_event_module_init(void)
{
    ...
    /* sonymiscにCharacterDeviceとして登録 */
    res = sonymisc_register( &usb_event_device );
    if(res){
        PINFO(" -->fail!!\n");
        PERR("my_init_module() failed\n");
        return -EIO;
    }
...
static void __exit usb_event_module_exit(void)
{
    ...
    /* sonymiscから登録削除 */
    res = sonymisc_deregister( &usb_event_device );
...
#if !(defined(CONFIG_ARCH_CXD4105BASED) && defined(CONFIG_USB_EVENT))
  #if !defined(CONFIG_USB_EVENT_MODULE)
   #error "Error: Kconfig"
  #endif
  module_init(usb_event_module_init);
#endif
