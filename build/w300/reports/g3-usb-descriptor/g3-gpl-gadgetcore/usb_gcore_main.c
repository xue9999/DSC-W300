 ...
 *---------------------------------------------------------------------------*/
#if !(defined(CONFIG_ARCH_CXD4105BASED) && defined(CONFIG_USB_GADGET_CORE))
  #if !defined(CONFIG_USB_GADGET_CORE_MODULE)
   #error "Error: Kconfig"
  #endif
  static
#endif
...
static struct sonymisc_device usb_gadgetcore_device = 
{
    .class  = DEV_CLASS_CHR,
    .minor  = USBGADGETCORE_MINOR_NUMBER,
    .name   = USBGADGETCORE_NAME,
    .fops   = &usb_gadgetcore_fops,
    .gd     = NULL,
};
...
g_bind(struct usb_gadget *gadget)
{
    ...
    if(!g_core->ep0req->buf){
        PDEBUG("usb_ep_alloc_buffer(EP0) ---> Fail\n");
        rc = -ENOMEM;
        usb_ep_free_request(g_core->ep0, g_core->ep0req);
        goto exit;
    }
    PDEBUG("usb_ep_alloc_buffer(EP0) ---> Success\n");
...
int __init usb_gcore_module_init(void)
{
    ...
    res = sonymisc_register( &usb_gadgetcore_device );
...
void __exit usb_gcore_module_exit(void)
{
    ...
    /* sonymiscから登録削除 */
    res = sonymisc_deregister( &usb_gadgetcore_device );
    if(res != 0){
        PERR("Error: Unregist from sonymisc\n");
    }
...
EXPORT_SYMBOL(usb_gadgetcore_stop_other_driver);
   ...
   #error "Error: Kconfig"
  #endif
  module_init(usb_gcore_module_init);
#endif
