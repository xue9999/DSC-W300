"""Decode four hash-pinned G3 routines as data; never execute firmware code."""
...
ROOT = HERE.parents[2]
sys.path.insert(0, str(HERE.parent / 're-tools' / 'site'))
...
SELECTED = {
    'libsencore.so': ['_Z5ParsePK12SenserPacket', '_Z17AdjustControlFuncPK12SenserPacket', 'local-category-helper'],
    'libadj33.so': ['command0001'],
    'libadj36.so': ['command0001'],
}

inventory = json.loads((HERE / 'retained-service-inventory.json').read_text())
...
destination = HERE / 'retained-service-dispatch.asm.txt'
